...
use datafusion::sql::sqlparser::ast::{
    AddDropSync, AlterColumnOperation, AlterTableOperation, Analyze, BinaryLength, CharacterLength,
    ColumnDef, ColumnOption, CreateFunction, CreateFunctionBody, CreateTable, CreateTableOptions,
    CreateView, Delete, Expr as SqlExpr, FromTable, FunctionBehavior, FunctionReturnType, Ident,
    Insert, Merge, Msck, ObjectName, ObjectType, Partition as SqlPartition, RenameTableNameKind,
    Reset, ResetStatement, Set, ShowCreateObject, SqlOption, Statement, TableFactor, TableObject,
    Truncate, Update, Use, Value as SqlValue,
};
...
impl SQLContext {
    ...
    pub async fn sql(&self, sql: &str) -> DFResult<DataFrame> {
        ...
        if let Some(show_partitions) = parse_show_partitions(&rewritten_sql)? {
            return self
                .handle_show_partitions(&show_partitions, enable_ident_normalization)
                .await;
        }
        ...
        match &statements[0] {
            ...
            Statement::Msck(msck) => self.handle_msck(msck).await,
            Statement::Analyze(analyze) => {
                self.handle_analyze(analyze, enable_ident_normalization)
                    .await
            }
    ...
    async fn handle_drop_partition(
        &self,
        catalog: &Arc<dyn Catalog>,
        identifier: &Identifier,
        ...
        if table.has_catalog_managed_partitions() {
            return self
                .drop_catalog_managed_partitions(
                    catalog,
                    identifier,
                    &table,
                    requests,
    ...
    async fn handle_add_partitions(
        &self,
        catalog: &Arc<dyn Catalog>,
        identifier: &Identifier,
    ...
    async fn handle_msck(&self, msck: &Msck) -> DFResult<DataFrame> {
        if !msck.repair {
            return Err(DataFusionError::Plan(
                "MSCK requires the REPAIR keyword".to_string(),
            ));
        }
        Self::ensure_partition_command_target(&msck.table_name, "MSCK REPAIR TABLE")?;
        let (catalog, _catalog_name, identifier) =
            self.resolve_catalog_and_table(&msck.table_name)?;
        let table = catalog
            .get_table(&identifier)
            .await
            .map_err(to_datafusion_error)?;
        ...
        let mode = match msck.partition_action {
            None | Some(AddDropSync::ADD) => crate::format_partition_repair::RepairMode::Add,
            Some(AddDropSync::DROP) => crate::format_partition_repair::RepairMode::Drop,
            Some(AddDropSync::SYNC) => crate::format_partition_repair::RepairMode::Sync,
        };
        crate::format_partition_repair::repair(catalog.as_ref(), &identifier, &table, mode)
            .await
            .map_err(to_datafusion_error)?;
        ok_result(&self.ctx)
    }
    ...
    async fn handle_analyze(
        ...
        Self::ensure_partition_command_target(table_name, "ANALYZE TABLE")?;
        let (catalog, _catalog_name, identifier) = self.resolve_catalog_and_table(table_name)?;
        let table = catalog
            .get_table(&identifier)
            .await
        ...
        let statistics = FormatTablePartitionStatsCollector::new(
            &table,
            !analyze.noscan,
            self.format_table_statistics_parallelism(&table),
        )
        .collect(&specs)
        .await
        .map_err(to_datafusion_error)?;
    ...
    /// `format-table.statistics.parallelism`, from the session (`SET paimon.<key>`) or the table,
    /// default 8. A value below one is read as one.
    fn format_table_statistics_parallelism(&self, table: &paimon::Table) -> usize {
        const KEY: &str = "format-table.statistics.parallelism";
        let session = self.dynamic_options.read().unwrap().get(KEY).cloned();
        session
            .or_else(|| table.schema().options().get(KEY).cloned())
            .and_then(|value| value.trim().parse::<i64>().ok())
            .map(|value| value.max(1) as usize)
            .unwrap_or(8)
    }

    async fn handle_show_partitions(
        &self,
        show_partitions: &ShowPartitionsStatement,
        enable_ident_normalization: bool,
    ) -> DFResult<DataFrame> {
        Self::ensure_partition_command_target(&show_partitions.table_name, "SHOW PARTITIONS")?;
        let (catalog, _catalog_name, identifier) =
            self.resolve_catalog_and_table(&show_partitions.table_name)?;
        let table = catalog
            .get_table(&identifier)
            .await
            .map_err(to_datafusion_error)?;
        ...
        let filter = if show_partitions.partition_filter.is_empty() {
            ...
            Some(normalize_catalog_partition_spec(&spec, &table, false)?)
        };
            ...
            .map_err(to_datafusion_error)?
        {
            ...
            let display_spec = display_partition_spec(&normalized, &table)?;
    ...
    fn ensure_partition_command_target(name: &ObjectName, operation: &str) -> DFResult<()> {
        let parsed = Self::parse_target_object_name(name)?;
        if let Some(branch) = parsed.branch() {
            return Err(DataFusionError::NotImplemented(format!(
                "{operation} on Paimon branch '{branch}' is not supported"
...
#[derive(Debug)]
struct ShowPartitionsStatement {
    table_name: ObjectName,
    partition_filter: Vec<SqlExpr>,
}

fn parse_show_partitions(sql: &str) -> DFResult<Option<ShowPartitionsStatement>> {
...
fn analyze_partition_prefix(
    ...
    let spec = parse_format_partition_spec(&assignments, table, false, enable_ident_normalization)?;
...
fn parse_format_partition_spec(
    ...
    for expr in exprs {
        let (column, value) = partition_assignment(expr, enable_ident_normalization)?;
        if !partition_keys.contains(&column) {
            return Err(DataFusionError::Plan(format!(
                "Column '{column}' is not a partition column"
            )));
        ...
        let value = match parse_format_partition_literal(value, field.data_type())? {
            ...
            .ok_or_else(|| {
                DataFusionError::NotImplemented(format!(
                    "Partition literals of type {:?} are not supported",
                    field.data_type()
                ))
            })?,
        };
...
fn parse_format_partition_literal(
    expr: &SqlExpr,
    data_type: &PaimonDataType,
) -> DFResult<Option<Datum>> {
    if matches!(
        expr,
        SqlExpr::Value(value) if matches!(&value.value, SqlValue::Null)
    ) {
        return Ok(None);
    }

    let mut value = format_partition_literal_as_string(expr)?;
    if !matches!(
        data_type,
    ...
    let arrow_type = paimon::arrow::paimon_type_to_arrow(data_type).map_err(to_datafusion_error)?;
    let scalar = ScalarValue::Utf8(Some(value))
        .cast_to(&arrow_type)
        .map_err(|error| {
            DataFusionError::Plan(format!(
                "Cannot convert partition literal {expr} to {data_type:?}: {error}"
            ))
        })?;
    scalar_to_datum(&scalar, data_type)
        .map(Some)
        .ok_or_else(|| {
            DataFusionError::Plan(format!(
                "Partition literals of type {data_type:?} are not supported"
            ))
        })
}

fn format_partition_literal_as_string(expr: &SqlExpr) -> DFResult<String> {
    match expr {
        SqlExpr::TypedString(typed)
            if matches!(
                typed.data_type,
                datafusion::sql::sqlparser::ast::DataType::Date
            ) =>
        {
            typed
                .value
                ...
                .clone()
                .into_string()
                .ok_or_else(|| DataFusionError::Plan(format!("Invalid partition literal: {expr}")))
        }
        SqlExpr::Value(value) => match &value.value {
            SqlValue::Number(value, _) => Ok(canonicalize_partition_number(value)),
            SqlValue::Boolean(value) => Ok(value.to_string()),
            SqlValue::Null => Err(DataFusionError::Internal(
                "NULL partition literal reached string conversion".to_string(),
            )),
            _ => value.value.clone().into_string().ok_or_else(|| {
                DataFusionError::Plan(format!("Unsupported partition literal: {expr}"))
            }),
        },
        SqlExpr::UnaryOp { op, expr }
            if matches!(
                op,
                datafusion::sql::sqlparser::ast::UnaryOperator::Plus
                    | datafusion::sql::sqlparser::ast::UnaryOperator::Minus
            ) =>
        {
            let SqlExpr::Value(value) = expr.as_ref() else {
                return Err(DataFusionError::Plan(format!(
                    "Unary signs are supported only for numeric partition literals: {expr}"
                )));
            };
            ...
            let sign = if matches!(op, datafusion::sql::sqlparser::ast::UnaryOperator::Minus) {
                "-"
            } else {
                ""
            };
            Ok(format!("{sign}{}", canonicalize_partition_number(value)))
        }
        _ => Err(DataFusionError::Plan(format!(
...
fn canonicalize_partition_number(value: &str) -> String {
    value
        .parse::<i128>()
        .map(|value| value.to_string())
        .or_else(|_| value.parse::<u128>().map(|value| value.to_string()))
...
fn normalize_catalog_partition_spec(
    spec: &HashMap<String, String>,
    table: &paimon::Table,
    require_complete: bool,
) -> DFResult<HashMap<String, Option<Datum>>> {
    let partition_keys = table.schema().partition_keys();
    if spec.keys().any(|key| !partition_keys.contains(key)) {
        return Err(DataFusionError::Plan(format!(
    ...
    if require_complete
        && (spec.len() != partition_keys.len()
            || partition_keys.iter().any(|key| !spec.contains_key(key)))
    {
        return Err(DataFusionError::Plan(format!(
            "Invalid partition spec {spec:?} for table {}: expected keys {partition_keys:?}",
            table.identifier().full_name()
        )));
    }
    ...
    let mut normalized = HashMap::with_capacity(spec.len());
    for key in partition_keys.iter().filter(|key| spec.contains_key(*key)) {
        let raw = &spec[key];
        let value = if raw == default_partition_name {
            None
        } else {
            let field = fields.get(key).ok_or_else(|| {
                DataFusionError::Plan(format!(
                    "Partition column '{key}' is missing from table schema"
                ))
            })?;
            Some(
                parse_format_partition_value(raw, field.data_type()).ok_or_else(|| {
                    DataFusionError::Plan(format!(
...
fn display_partition_spec(
    spec: &HashMap<String, Option<Datum>>,
    table: &paimon::Table,
) -> DFResult<HashMap<String, String>> {
    let fields = table
        .schema()
        ...
        .map(|(key, value)| {
            let value = match value {
                ...
                Some(datum) => {
                    ...
                    format_partition_value(datum, field.data_type(), default_partition_name, false)
                        .ok_or_else(|| {
                            DataFusionError::NotImplemented(format!(
                                "SHOW PARTITIONS does not support type {:?}",
                                field.data_type()
                            ))
                        })?
                }
...
fn sql_expr_to_datum(expr: &SqlExpr, data_type: &PaimonDataType) -> DFResult<Datum> {
    if let SqlExpr::TypedString(typed) = expr {
        if matches!(
            typed.data_type,
            datafusion::sql::sqlparser::ast::DataType::Date
        ) && matches!(data_type, PaimonDataType::Date(_))
        {
            if let SqlValue::SingleQuotedString(value) = &typed.value.value {
                return parse_date_datum(value);
            }
        }
        return Err(DataFusionError::Plan(format!(
            "Cannot convert {expr} to {data_type:?}"
        )));
    }
    ...
    match (value, data_type) {
        ...
        (SqlValue::SingleQuotedString(s), PaimonDataType::Char(_) | PaimonDataType::VarChar(_))
            if !negate =>
        {
...
fn parse_date_datum(value: &str) -> DFResult<Datum> {
    let date = chrono::NaiveDate::parse_from_str(value, "%Y-%m-%d")
        .map_err(|e| DataFusionError::Plan(format!("Invalid DATE '{value}': {e}")))?;
    let unix_epoch =
        chrono::NaiveDate::from_ymd_opt(1970, 1, 1).expect("1970-01-01 is a valid date");
    let epoch_days = i32::try_from((date - unix_epoch).num_days()).map_err(|e| {
        DataFusionError::Plan(format!(
            "DATE '{value}' is outside the supported partition range: {e}"
        ))
    })?;
    Ok(Datum::Date(epoch_days))
}
