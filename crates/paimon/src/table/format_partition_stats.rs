...
use crate::spec::{Partition, PartitionStatistics};
...
impl<'a> FormatTablePartitionStatsCollector<'a> {
    ...
    pub async fn collect(
        ...
        let options = self
            .table
            .rest_env()
            .and_then(|rest_env| rest_env.catalog_managed_partition_options())
        ...
        let table_path = options.table_path.trim_end_matches('/');
        ...
        let listings: Vec<Vec<FileStatus>> = futures::stream::iter(directories)
            .map(|directory| async move {
                list_format_table_data_files(file_io, &directory, format_extension).await
            })
